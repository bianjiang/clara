...
Ext.ux.Printer = function() {
	...
	return {
		 ...
		 */
		print: function(component,config) {
			...
			for (var i = xtypes.length - 1; i >= 0; i--){
				...
				if (renderer != undefined) {
					renderer.print(component, config);
					break;
				}
			}
		}
...
Ext.ux.Printer.BaseRenderer = Ext.extend(Object, {
	 ...
	 */
	print: function(component, config) {
		var name = (config && typeof config.title != "undefined")?config.title:((component && component.getXType)
		? String.format("print_{0}_{1}", component.getXType(), component.id)
				: "print");

		var win = window.open('');

		win.document.write(this.generateHTML(component, "<h1>"+name+"</h1>"));
		win.document.close();

		win.print();
		if (!(config && config.keepWindowOpen)) win.close();
	},
	 ...
	 * @param {Ext.Component} header HTML to add to top of page
	 * @return {String} An HTML fragment to be placed inside the print window
	 */
	generateHTML: function(component, header) {

		var stylesheetHtml = '<link href="' + this.stylesheetPath + '" rel="stylesheet" type="text/css" media="screen,print" />';

		for (var i=0,l=this.customStylesheetPaths.length;i<l;i++){
			stylesheetHtml += '<link href="' + this.customStylesheetPaths[i] + '" rel="stylesheet" type="text/css" media="screen,print" />';
		}
	 ...
	 */
	stylesheetPath: appContext+'/static/styles/ext.ux.printer.css',
	/**
	 * @property customStylesheetPaths
	 * @type array
	 * A list of paths at which custom print stylesheets can be found
	 */
	customStylesheetPaths: [ 
	                        appContext+'/static/styles/application.css',
	                        appContext+'/static/styles/protocol/protocol-form/review/review.css',
	                        appContext+'/static/styles/uams.ext.print.css'
	                        ]
...
Ext.ux.Printer.GridPanelRenderer = Ext.extend(Ext.ux.Printer.BaseRenderer, {
		...
		grid.store.data.each(function(item, row) {
			...
			Ext.iterate(item.data, function(key, value) {
				...
				if (column && column.dataIndex == key) {
						...
						return false;
				} else if (column && column.id === 'numberer'){
                    convertedData[column.id] = (row + 1);
                }
				}, this);
	...
	getColumns: function(grid) {
		...
		Ext.each(grid.getColumnModel().getColumnsBy(function(c){return !c.hidden;}), function(col) {
			columns.push(col);
		}, this);
