...
Ext.ux.Exporter = function() {
  return {
    ...
    exportGrid: function(grid, formatter, config) {
      ...
      if (config.bypassEncode) return formatter.format(grid.store, config);
      else return Base64.encode(formatter.format(grid.store, config));
    ...
    exportStore: function(store, formatter, config) {
       ...
       if (config.bypassEncode) return formatter.format(store, config);
       else return Base64.encode(formatter.format(store, config));
  ...
  buildHeader: function() {
    ...
    Ext.each(this.columns, function(col) {
      ...
      if (col.header != undefined) {
        ...
        title = (typeof col.name != 'undefined')?col.name.replace(/_/g, " "):"Column";
...
Ext.ux.Exporter.ExcelFormatter.Cell = Ext.extend(Object, {
  ...
  render: function() {
    // mbaker@uams.edu: strip out dash cell html (needs to be regex or custom exporter)
	  if (this.type == "String") this.value = escape(this.value).replace(/\//g,"").replace(/%/g,"");
