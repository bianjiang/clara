...
Clara.ContractForm.Rule = function(o){
	this.id=							(o.id || Ext.id());	
	...
	this.doesAnswerContains = (o.doesAnswerContains || function (answer, checkValues){
		...
		for(var i = 0; i < l; i ++) {			
			...
			if (jQuery.inArray(checkValues[i], answerInc) > -1) {
...
Clara.ContractForm.Rules = {
		...
		getAnswersForPaths: function(paths){
			...
			var url = appContext + "/ajax/contracts/" + claraInstance.id + "/contract-forms/" + claraInstance.form.id + "/contract-form-xml-datas/" + claraInstance.form.xmlDataId + "/xml-elements/listValues";
			jQuery.ajax({
				type: 'POST',
				url: url,
				async:false,
...
function processDependencies(a,b){
	clog("[DEPRECATED] rules.js: processDependencies() has been replaced with Clara.ContractForm.Rules.processPage()");
	Clara.ContractForm.Rules.processPage();
}