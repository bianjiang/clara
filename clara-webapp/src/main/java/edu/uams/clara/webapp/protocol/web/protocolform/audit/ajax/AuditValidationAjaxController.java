package edu.uams.clara.webapp.protocol.web.protocolform.audit.ajax;
...
@Controller
public class AuditValidationAjaxController {
	
	private final static Logger logger = LoggerFactory
	.getLogger(AuditValidationAjaxController.class);
	...
	@RequestMapping(value = "/ajax/protocols/{protocolId}/protocol-forms/{protocolFormId}/audit/protocol-form-xml-datas/{protocolFormXmlDataId}/validate", method = RequestMethod.GET)
	public @ResponseBody
	List<ValidationResponse> validateProtocolContinuingReviewForm(
		...
		if(StringUtils.hasText(xmldata)){
			...
			List<Rule> auditValidationRules = getValidationRuleContainer().getValidationRules("auditValidationRules");

			Assert.notNull(auditValidationRules);
			
			Set<String> valueKeys = getValidationRuleContainer().getCachedValueKeys("auditValidationRules");
