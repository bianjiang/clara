package edu.uams.clara.webapp.protocol.web.protocolform.hud.ajax;
...
@Controller
public class HUDValidationAjaxController {
	
	private final static Logger logger = LoggerFactory
	.getLogger(HUDValidationAjaxController.class);
	...
	@RequestMapping(value = "/ajax/protocols/{protocolId}/protocol-forms/{protocolFormId}/humanitarian-use-device/protocol-form-xml-datas/{protocolFormXmlDataId}/validate", method = RequestMethod.GET)
	public @ResponseBody
	List<ValidationResponse> validateHUDForm(
		...
		if(StringUtils.hasText(xmldata)){
			...
			List<Rule> protocolValidationRules = getValidationRuleContainer().getValidationRules("hudValidationRules");

			Assert.notNull(protocolValidationRules);

			Set<String> valueKeys = getValidationRuleContainer().getCachedValueKeys("hudValidationRules");
