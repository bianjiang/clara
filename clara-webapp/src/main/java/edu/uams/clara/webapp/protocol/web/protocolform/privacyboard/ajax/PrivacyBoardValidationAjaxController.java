package edu.uams.clara.webapp.protocol.web.protocolform.privacyboard.ajax;
...
@Controller
public class PrivacyBoardValidationAjaxController {
	...
	@RequestMapping(value = "/ajax/protocols/{protocolId}/protocol-forms/{protocolFormId}/privacy-board/protocol-form-xml-datas/{protocolFormXmlDataId}/validate", method = RequestMethod.GET)
	public @ResponseBody
	List<ValidationResponse> validatePrivacyBoardForm(
		...
		if(StringUtils.hasText(xmldata)){
			...
			List<Rule> protocolValidationRules = getValidationRuleContainer().getValidationRules("privacyBoardValidationRules");

			//Assert.notNull(protocolValidationRules);

			Set<String> valueKeys = getValidationRuleContainer().getCachedValueKeys("privacyBoardValidationRules");
