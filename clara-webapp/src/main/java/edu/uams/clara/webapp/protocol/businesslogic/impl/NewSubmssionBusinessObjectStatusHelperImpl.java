...
public class NewSubmssionBusinessObjectStatusHelperImpl extends
		ProtocolBusinessObjectStatusHelper {
	private final static Logger logger = LoggerFactory
			.getLogger(NewSubmssionBusinessObjectStatusHelperImpl.class);
	...
	public NewSubmssionBusinessObjectStatusHelperImpl()
...
//					}
					...
					/*
					if (selectedCommittees.contains(Committee.PHARMACY_REVIEW)){
						ProtocolFormCommitteeStatus pharmacyReviewStatus = this
								.getProtocolFormCommitteeStatusDao()
								.getLatestByCommitteeAndProtocolFormId(
										Committee.PHARMACY_REVIEW,
										protocolForm.getId());
						
						if (pharmacyReviewStatus == null){
							type = "NEED_PHARMACY_REVIEW";
						}else if (!pharmacyReviewStatus.equals(ProtocolFormCommitteeStatusEnum.IN_REVIEW_REQUESTED) && !pharmacyReviewStatus.equals(ProtocolFormCommitteeStatusEnum.IN_WAIVER_REQUESTED)){
							selectedCommittees.remove(Committee.PHARMACY_REVIEW); 
							type = "REQUESTED_PHARMACY_REVIEW_COMPLETED";
						}
					}*/
			...
			if(committee.equals(Committee.BUDGET_MANAGER) && ProtocolFormStatusEnum.UNDER_BUDGET_MANAGER_REVIEW.equals(protocolFormStatus)){
				NodeList budgetReviewInvovledCommittees = (NodeList) xPath
						.evaluate(
								"//invovled-committees/committee",
								extraDataXmlDoc,
								XPathConstants.NODESET);
				
				List<Committee> budgetReviewSelectedCommittees = new ArrayList<Committee>();

				for (int j = 0; j < budgetReviewInvovledCommittees.getLength(); j++) {
					Element budgetReviewInvovledCommitteeEl = (Element) budgetReviewInvovledCommittees
							.item(j);
					logger.debug("preProcessCommitteeReviewXml->budget-review-invovled-committee: "
							+ budgetReviewInvovledCommitteeEl.getTextContent()
							+ "; type: "
							+ budgetReviewInvovledCommitteeEl
									.getAttribute("type"));

					Committee budgetReviewInvolvedCommittee = Committee
							.valueOf(budgetReviewInvovledCommitteeEl
									.getTextContent());

					budgetReviewSelectedCommittees.add(budgetReviewInvolvedCommittee);
				}
			...
			if (committee.equals(Committee.IRB_EXPEDITED_REVIEWER) && action.equals("APPROVE")) {
				...
				try {
					...
					recentMotion = xmlHandler.getSingleStringValueByXPath(form.getObjectMetaData(), "/protocol/summary/irb-determination/recent-motion");
				} catch (Exception e) {
					e.printStackTrace();
				}
				
				//If it's an approval of minor met, system should not change the adult and ped risk determined in the meeting.
				if (recentMotion.equals("Defer with minor contingencies")) return extraDataXml;
				
				Node expeditedExtraContentNode = (Element) xPath
						.evaluate(
								"/committee-review/committee/extra-content/expedited",
								extraDataXmlDoc,
								XPathConstants.NODE); 
				...
				try {
					...
					if (ageRangeLst != null && !ageRangeLst.isEmpty()) {
						...
						expeditedExtraContentNode.appendChild(adultRiskEl);
						...
						expeditedExtraContentNode.appendChild(pedRiskEl);
					}
				} catch (Exception e) {
				...
				if (action.equals("ASSIGN_REVIEWER")){
					ProtocolFormCommitteeStatus budgetManagerCommitteeStatus = getProtocolFormCommitteeStatusDao()
						...
						if (ProtocolFormCommitteeStatusEnum.PENDING_REVIEWER_ASSIGNMENT.equals(budgetManagerCommitteeStatus.getProtocolFormCommitteeStatus())){
							if (protocolFormReviewLogicServiceContainer.getProtocolFormReviewLogicService("NEW_SUBMISSION").isInvolvedByType(protocolForm, "Device")){
