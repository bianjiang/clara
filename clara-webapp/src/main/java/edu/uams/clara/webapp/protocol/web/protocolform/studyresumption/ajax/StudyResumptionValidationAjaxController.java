package edu.uams.clara.webapp.protocol.web.protocolform.studyresumption.ajax;
...
@Controller
public class StudyResumptionValidationAjaxController {
	...
	@RequestMapping(value = "/ajax/protocols/{protocolId}/protocol-forms/{protocolFormId}/study-resumption/protocol-form-xml-datas/{protocolFormXmlDataId}/validate", method = RequestMethod.GET)
	public @ResponseBody
	List<ValidationResponse> validateStudyResumptionForm(
		...
		if(StringUtils.hasText(xmldata)){
			...
			List<Rule> reportableNewInformationValidationRules = getValidationRuleContainer().getValidationRules("studyResumptionValidationRules");

			Assert.notNull(reportableNewInformationValidationRules);
			
			Set<String> valueKeys = getValidationRuleContainer().getCachedValueKeys("studyResumptionValidationRules");
