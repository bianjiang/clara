	...
	private void updateHospitalCost() {
		...
		for (HospitalChargeUpdate hospitalCharageUpdate : hospitalChargeUpdateList) {
			...
			try {
				...
				if(!hospitalChargeProcedure.isOverwritten()){
					hospitalChargeProcedure.setCost(cost);
				}
	...
	private void addHospitalCptToCPTTable(){
		...
		for(HospitalChargeProcedure hcp : hcps){
			...
			try{
				...
				cptCode.setShortDescription(hcp.getDescription());
				cptCodeDao.saveOrUpdate(cptCode);
				logger.debug("CPTCode updated: "+ cptCode.getCode());
			}catch(Exception e){
...
public class ImportHospitalChargeDataService {
	...
	private void updateHospitalCost() {
		...
		for (HospitalChargeUpdate hospitalCharageUpdate : hospitalChargeUpdateList) {
			...
			try {
				...
				//in case the lab fee is overwritten skip the update for existing cost when the update cost is 0
				if(!hospitalChargeProcedure.isOverwritten()&&cost.longValue()!=0){
					hospitalChargeProcedure.setCost(cost);
				}
	...
	private void updateCostForSIA(){
		try {
			CSVReader csvReader = new CSVReader(new FileReader(
					"C:\\Data\\hospitalCostforStatusA.csv"));
			...
			while ((nextLine = csvReader.readNext()) != null) {
				for (int i = 0; i < numberOfColumns; i++) {
					String cost = nextLine[1].trim();
					String cpt = nextLine[0].trim();
					...
					try{
						HospitalChargeUpdate hospitalChargeUpdate = hospitalChargeUpdateDao.findByCptCode(cpt).get(0);
						if(hospitalChargeUpdate.getSi().equals("A")){
							hospitalChargeUpdate.setCost(bdPrice);
							hospitalChargeUpdateDao.saveOrUpdate(hospitalChargeUpdate);
							logger.debug("status a update xxxxxxxxx "+cpt);
						}
					}catch(Exception ex){
						ex.printStackTrace();
					}
